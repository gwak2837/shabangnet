...
import {
  parseShoppingMallTemplateColumnConfig,
...
export async function analyzeShoppingMallFile({ file, headerRow }: AnalyzeInput): Promise<AnalyzeResult> {
  ...
  if (headerRow !== undefined) {
    ...
    if (headerRowIndex < 0 || headerRowIndex >= data.length) {
      throw new Error('헤더 행 번호가 올바르지 않아요')
    }
  } else {
