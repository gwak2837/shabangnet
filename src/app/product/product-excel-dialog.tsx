...
import type { ProductExcelImportResult } from './product-excel.types'

import { importProductsExcel } from './excel-actions'
import { PRODUCT_EXCEL_HEADER } from './product-excel.types'

interface ProductExcelDialogProps {
  onOpenChange: (open: boolean) => void
  open: boolean
}

export function ProductExcelDialog({ open, onOpenChange }: ProductExcelDialogProps) {
  const [state, formAction, isPending] = useFormAction<ProductExcelImportResult | null, FormData>(
    importProductsExcel,
    null,
    {
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
          <ul className="list-disc pl-5 space-y-1">
            ...
            <li className="break-all">
              헤더 예시: <span className="font-mono">{PRODUCT_EXCEL_HEADER.join(' | ')}</span>
            </li>
          </ul>
        </div>
        ...
        <form action={formAction} className="flex flex-col gap-3">
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-slate-900" htmlFor="product-excel">
              엑셀 파일
            </label>
            <Input
